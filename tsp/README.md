...
Метод симуляции отжига, самый классический. Константы поперебирал, итоговые лежат в [anneal_solver](tsp/solver/anneal_solver.cpp) в функции `choose_constants`. Опять же для больших тестов ставлю константы поменьше чтобы укладываться в нормальное время. Также важная вещь наверное, для второй вершины для 2opt выбираю одну из 20-40 ближайших соседей, а не рандомную.
